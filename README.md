...
修改脚本中的 RGB_PATH、DEPTH_PATH 即可测试不同帧。  

## 批量处理与对比实验  
1. 颜色阈值法批量处理  
运行seg_compare/scripts/hsv_seg_mdf.py，生成 pred_color/ 目录下的掩码，输出 mIoU、Pixel Accuracy、Precision、Recall、F1 等指标。  

3. YOLOv8-seg 批量处理  
运行seg_compare/scripts/yolov8_seg.py，批量处理测试集图像，生成 pred_yolo/ 目录，输出 mIoU、Pixel Accuracy、Precision、Recall、F1 等指标。  

4. 生成对比图  
使用seg_compare/scripts/compare_vis.py 将原图、真实掩码、两种方法预测掩码并排显示，保存为 comparison.png。  
